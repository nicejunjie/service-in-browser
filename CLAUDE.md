...
- **2-finger tap** (no pinch movement) → dismiss the native keyboard.
...
- **Native keyboard, no button** — xpra's drawn `.simple-keyboard` is hidden; instead a **hidden real `<input>` (`#xpra-kbd`)** receives the actual iOS/Android keyboard. A 1-finger tap raises it (focusing the input within the tap gesture, like the terminal); a 2-finger tap dismisses it (a 500 ms post-blur window stops the dismissing tap from reopening it). Typed characters are forwarded to the remote as synthetic key events (xpra reads `event.code`/`key`/`keyCode`; its handlers don't check `isTrusted`), built by **diffing the input's value** on each `input` event so backspace and autocorrect replacements work; Enter/Tab/empty-backspace go via `keydown`. (Replaced the old floating `⌨` chip + xpra's JS keyboard.)
