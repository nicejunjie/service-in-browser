...
- **Native keyboard** — xpra's drawn `.simple-keyboard` is hidden; instead a **hidden real `<input>` (`#xpra-kbd`)** receives the actual iOS/Android keyboard. A `⌨` button raises/dismisses it. The button is **required**: iOS only opens the keyboard when you tap a genuine focusable element, so focusing the input from a bare canvas tap doesn't reliably work — the button's own clean `touchend`/`click` handler runs `focus()` inside a real user gesture. The window-capture touch layer (patch 4) **ignores taps on `#vkb-toggle`** (`onChip` guard) — otherwise its `stopPropagation` would kill the button's events. Typed characters are forwarded to the remote as synthetic key events (xpra reads `event.code`/`key`/`keyCode`; its handlers don't check `isTrusted`), built by **diffing the input's value** on each `input` event so backspace and autocorrect replacements work; Enter/Tab/empty-backspace go via `keydown`. (Replaces xpra's drawn JS keyboard; the button now triggers the native one.)
